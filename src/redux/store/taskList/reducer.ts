...
export default function taskListReducer(state = initialState, action: ActionTypes): TaskListState {
    switch (action.type) {
        case (TaskListActionTypes.CHANGE_TASK_STATUS):
            const clonedTaskList: TaskInfo[] = cloneDeep(state);
            const taskForUpdateIndex: number = state.findIndex((task) => task.id === action.taskId);
